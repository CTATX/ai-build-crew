      ...
      <section className={`estimator ${stage !== "estimate" ? "pre-estimate-hidden" : ""}`} id="estimator" aria-hidden={stage !== "estimate"}>
        ...
        <div className="workbench">
          <div className="inputs">
            ...
            <div className="field wide"><label>Required formats {assumptions.includes("modalities") && <em>ASSUMED</em>}</label><div className="multi-select" role="group" aria-label="Required formats">{[["text", "Text"], ["image", "Images"], ["audio", "Audio"], ["video", "Video"]].map(([value, label]) => <button type="button" key={value} className={modalities.includes(value) ? "selected" : ""} aria-pressed={modalities.includes(value)} onClick={() => { const next = modalities.includes(value) ? modalities.filter((item) => item !== value) : [...modalities, value]; if (next.length) { setModalities(next); setAssumptions((current) => current.filter((item) => item !== "modalities")); setSelected(null); setDecision("Not decided"); } }}>{label}</button>)}</div><small className="field-hint">Select every format the same workflow must understand. A model must support all selected formats to remain eligible.</small></div>
