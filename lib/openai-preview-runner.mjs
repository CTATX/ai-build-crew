...
export const OPENAI_PREVIEW_VERSION = "openai-preview-runner-1.0.0";
...
export async function runOpenAIPreview({ apiKey, ceilingUsd = 1, fetchImpl = fetch, cases = inventoryAssistantCases, modelId = OPENAI_PREVIEW_MODEL } = {}) {
  ...
  for (const evaluationCase of cases) {
    ...
    results.push(Object.freeze({
      ...
      retryCount: 0,
      deterministicScore: Object.freeze({ passed: passedTerms.length === evaluationCase.requiredTerms.length, termsFound: passedTerms.length, termsRequired: evaluationCase.requiredTerms.length }),
  ...
  return Object.freeze({
    ...
    casesPassed: results.filter((item) => item.deterministicScore.passed).length,
