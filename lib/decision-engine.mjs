...
export const ENGINE_VERSION = "decision-engine-1.4.1";
...
export function suggestWorkflow({ task, risk, dataSensitivity, regulated }) {
  const primaryByTask = {
    "Classification & extraction": 1,
    "Content & summarization": 1,
    "Product analysis": 2,
  ...
  const primarySteps = primaryByTask[task] ?? 2;
  const checkerSteps = risk === "High" || regulated || ["Sensitive", "Protected"].includes(dataSensitivity)
    ? 2
    : risk === "Low"
      ? 0
      : 1;
  return Object.freeze({
    primarySteps,
    checkerSteps,
    rationale: `${primarySteps} primary step${primarySteps === 1 ? "" : "s"} for ${task.toLowerCase()} and ${checkerSteps} checker step${checkerSteps === 1 ? "" : "s"} for the stated consequence and data boundary.`,
    evidence: "DETERMINISTIC PLANNING RULE — CONFIRM OR EDIT",
  });
}
...
export function evaluateDecision(workload, recommendation, scenarios, costEvaluation) {
  ...
  const checks = [
    ...
    { id: "EVAL-003", name: "Recommendation supports every required format", pass: workload.modalities.every((modality) => recommendation.modalities.includes(modality)) },
...
export function analyzeWorkload(input) {
  ...
  const requestedModalities = Array.isArray(input.modalities) && input.modalities.length
    ? [...new Set(input.modalities)].sort()
    : [input.modality ?? "text"];
  const workflowSuggestion = suggestWorkflow(input);
  const primaryStepsSuggested = !Number.isFinite(Number(input.primarySteps)) || Number(input.primarySteps) <= 0;
  const checkerStepsSuggested = !Number.isFinite(Number(input.checkerSteps)) || Number(input.checkerSteps) < 0;
  const workload = Object.freeze({
    task: input.task,
    risk: input.risk,
    dataSensitivity: input.dataSensitivity,
    modalities: Object.freeze(requestedModalities),
    modality: requestedModalities[0],
    ...
    primarySteps: primaryStepsSuggested ? workflowSuggestion.primarySteps : Math.min(50, Math.max(1, Number(input.primarySteps))),
    checkerSteps: checkerStepsSuggested ? workflowSuggestion.checkerSteps : Math.min(10, Math.max(0, Number(input.checkerSteps))),
  ...
  const catalogEligible = models.filter((model) => workload.modalities.every((modality) => model.modalities.includes(modality)) && workload.inputTokens <= model.contextTokens);
  ...
  return Object.freeze({ workload, workflowSuggestion: Object.freeze({ ...workflowSuggestion, applied: primaryStepsSuggested || checkerStepsSuggested, primaryStepsSuggested, checkerStepsSuggested }), required, catalogEligible, eligible, ranked, recommendation, scenarios, costEvaluation, evaluation, governance: governanceResult, audit, assumptions, disposition, coverage, catalog });
}
...
export function deterministicBrief(result) {
  if (!result.recommendation) return result.workload.budget > 0 && result.governance.findings.some((finding) => finding.id === "GOV-011")
    ...
    : result.catalogEligible.length
      ? `${result.catalogEligible.map((model) => model.name).join(" and ")} list the required formats and context in published catalog data, but no candidate is evaluation-ready for quality tier ${result.required}. No fallback cost is emitted. Run the shared workload evaluation or split the work into separately estimated specialist stages.`
      : `No model in the current catalog lists every required format (${result.workload.modalities.join(" + ")}) for one model step. No fallback cost is emitted. Revise the requirement or split the work into separately estimated specialist stages.`;
  ...
  return `${result.recommendation.name} is the least expensive rule-eligible model in catalog ${result.catalog.version} for quality tier ${result.required} and the required ${result.workload.modalities.join(" + ")} format${result.workload.modalities.length === 1 ? "" : "s"}. Likely token cost is $${result.scenarios.expected.perCompletedTask.toFixed(4)} per completed task, inside a $${result.scenarios.low.perCompletedTask.toFixed(4)}–$${result.scenarios.high.perCompletedTask.toFixed(4)} planning spread${delta > 0 ? `; the next eligible option costs $${delta.toFixed(4)} more per task` : ""}. Output length and retries are model-profile assumptions, not guarantees.${coverage}${review}`;
}
