...
export const ENGINE_VERSION = "decision-engine-1.0.0";
...
export function analyzeWorkload(input) {
  ...
  const eligible = models.filter((model) => model.quality >= required && model.modalities.includes(workload.modality) && workload.inputTokens <= model.contextTokens);
  const ranked = eligible.map((model) => ({ model, expected: costFor(model, workload, 1) })).sort((a, b) => a.expected.monthly - b.expected.monthly || a.model.id.localeCompare(b.model.id));
  ...
  return Object.freeze({ workload, required, eligible, ranked, recommendation, scenarios, evaluation, governance: governanceResult, audit, assumptions, disposition, catalog });
}
...
export function deterministicBrief(result) {
  if (!result.recommendation) return "No model in the current catalog clears the rule gates. Stop and escalate rather than force a choice.";
  ...
  return `${result.recommendation.name} is the least expensive rule-eligible model in catalog ${result.catalog.version}. Expected token cost is $${result.scenarios.expected.monthly.toFixed(2)} per month${delta > 0 ? `; the next eligible option costs $${delta.toFixed(2)} more` : ""}.${review}`;
}
